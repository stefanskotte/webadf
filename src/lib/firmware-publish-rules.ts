...
export type PublishRefusalReason =
  ...
  | 'semver_regression'
  | 'bad_semver'
  // The signature covers the sequence (spec D4): a sequence that moved
  // between signing and recording (firmware-releases.ts's publishRelease)
  // would publish a signature that never verifies. `detail` carries the
  // before/after numbers -- kept out of the reason itself so this stays a
  // closed, matchable union.
  | 'sequence_moved';

export class PublishRefused extends Error {
  constructor(
    public readonly reason: PublishRefusalReason,
    public readonly detail?: string,
  ) {
    super(detail ? `firmware publish refused: ${reason}: ${detail}` : `firmware publish refused: ${reason}`);
