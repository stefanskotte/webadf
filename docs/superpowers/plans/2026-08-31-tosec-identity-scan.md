...
export function DatUpload() {
  ...
  async function onChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error('Could not import that DAT', {
          description: body.error === 'no_entries'
            ? 'It parsed, but contained no entries — is it really a TOSEC DAT?'
            : `The server answered ${res.status}.`,
        });
        return;
      }
      toast.success(`Imported ${body.imported} entries`, { description: body.setName });
      router.refresh();
    } finally {
      setBusy(false);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <span className="text-[12px]" style={{ color: 'var(--muted)' }}>
        {busy ? 'Importing…' : 'ClrMamePro or XML. Re-importing a set updates it in place.'}
      </span>
    </div>
  );
}
