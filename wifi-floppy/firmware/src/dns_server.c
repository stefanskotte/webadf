...
// network" instead of reporting a network with no internet.
//
...
// Fixed-size answer record this responder always emits: a compressed name
...
int dns_handle(const uint8_t *req, int len, uint8_t *out, int cap) {
    ...
    // copy from offset 0 through qend), then one fixed-size answer record.
    int reply_len = qend + DNS_ANSWER_LEN;
    ...
    out[2] = 0x81;
    out[3] = 0x80;
    // ANCOUNT = 1 (NSCOUNT/ARCOUNT already 0 from the copied header, since
    // the offer this responder makes never sets them).
